...
import { TaxonomyList } from "../../../components/TaxonomyList/TaxonomyList";
 ...
 * @param {boolean} [listMode=false]    - Render all leaf items as a searchable, clickable list sorted by most-recently-used (persisted per tag in localStorage) instead of the default dropdown/tree UI. Use when you have many flat labels and want to click instead of search-then-click.
 */
const TagAttrs = types.model({
  ...
  listmode: types.optional(types.boolean, false),
});

const MRU_STORAGE_PREFIX = "lsf-taxonomy-mru:";
const MRU_MAX_ENTRIES = 200;

const loadMruFromStorage = (tagName) => {
  try {
    const raw = localStorage.getItem(MRU_STORAGE_PREFIX + tagName);
    if (!raw) return [];
...
const saveMruToStorage = (tagName, paths) => {
  try {
    localStorage.setItem(MRU_STORAGE_PREFIX + tagName, JSON.stringify(paths));
  } catch (e) {
...
const Model = types
  ...
  .volatile(() => ({
    ...
    mruPaths: [], // most-recently-used path keys for listMode, persisted to localStorage
  }))
  .views((self) => ({
    get children() {
  ...
  .actions((self) => ({
    afterAttach() {
      // Load per-tag MRU ordering from localStorage for listMode.
      // Safe to run regardless of listMode — the list is only read by the
      // list renderer, so this is a cheap no-op for default mode.
      if (self.listmode && self.name) {
        self.mruPaths = loadMruFromStorage(self.name);
        // listMode is designed around a "draw, then pick" workflow, so a newly
        // drawn region must become the current (highlighted) region immediately.
        // Force the global "selectAfterCreate" setting on when this tag mounts
        // so the per-region list binds to the just-drawn shape without an extra
        // click. This overrides any stale localStorage value from prior sessions.
        const settings = self.annotation?.store?.settings;
        if (settings && settings.selectAfterCreate === false) {
    ...
    /**
     * Bump a path key to the front of the MRU list and persist to localStorage.
     * Called by the TaxonomyList component when the user clicks an item.
     * @param {string} pathKey path joined by the tag's pathseparator
     */
    recordMruUse(pathKey) {
      if (!pathKey) return;
      const next = [pathKey, ...self.mruPaths.filter((k) => k !== pathKey)];
      if (next.length > MRU_MAX_ENTRIES) next.length = MRU_MAX_ENTRIES;
      self.mruPaths = next;
      if (self.name) saveMruToStorage(self.name, next);
    },
  }))
  .actions((self) => {
    const Super = {
...
const HtxTaxonomy = observer(({ item }) => {
  ...
  // In list mode we want the list to stay visible even when no region is selected,
  // so we bypass perRegionVisible() and only respect the general isVisible flag.
  // Clicking a list item when there's no region is handled by the list component itself.
  const listMode = item.listmode;
  const shouldShow = listMode ? item.isVisible : item.perRegionVisible() && item.isVisible;
  const visibleStyle = shouldShow ? {} : { display: "none" };
  const options = {
    ...
    <div className={className} style={visibleStyle} ref={item.elementRef}>
      {listMode ? (
        <TaxonomyList
          ...
          mruPaths={item.mruPaths}
          onChange={item.onChange}
          onUse={item.recordMruUse}
          hasSelectableRegion={Boolean(!item.perregion || item.annotation?.highlightedNode)}
          options={options}
